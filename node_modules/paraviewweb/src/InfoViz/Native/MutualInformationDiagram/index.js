...
function informationDiagram(publicAPI, model) {
  ...
  publicAPI.render = () => {
    ...
    const outerHistoRadius = Math.min(width, height) / 2; 
    const veryOutermostRadius = outerHistoRadius * 1.31; 
    const histoRadius = outerHistoRadius *.92;
    const outerRadius = histoRadius *.70;  //.88
    const innerRadius = outerRadius * .80;    //.86
    const deltaRadius = outerRadius - innerRadius;
    ...
    const groupText = groupEnter
      ...
      .attr('dy', (outerRadius - innerRadius)*.66);
    ...
    const textPath = groupText
      ...
      .attr('startOffset', '25%')
      .attr('font-size',  (outerRadius - innerRadius)/2 + 'px')
